...
How Hously ships to production: one Docker image with both API and web, served via `SERVE_STATIC=true`, fronted by `docker-compose.prod.yml`.
...
- `GITHUB_RELEASES_REPO` — defaults to `samuelloranger/hously`; used by the Releases panel.
...
3. Falls through to a `GET *` catch-all that returns `index.html` with `<script>window.__HOUSLY_BOOTSTRAP__=…</script>` injected (`apps/api/src/index.ts:162-174`). This pre-populates the user session before the SPA boots.
...
services:
  hously:
    image: ghcr.io/samuelloranger/hously:latest
    env_file: .env
    environment:
      - SERVE_STATIC=true
    volumes:
...
- The qBittorrent webhook URL resolver (`apps/api/src/routes/integrations/qbittorrent/index.ts`) prefers internal Docker DNS (`http://hously:3000`) over the public URL — see comments in that file for the priority order.

## GitHub Releases

Production CI publishes images to `ghcr.io/samuelloranger/hously`. The in-app Settings → Releases panel polls GitHub at the cron pattern `0 */6 * * *` (see `apps/api/src/services/queueService.ts:setupScheduledJobs`, job `REFRESH_GITHUB_RELEASES`).
