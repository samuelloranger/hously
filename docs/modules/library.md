...
The heart of Hously's "\*arr replacement": movie + TV library with TMDB discovery, indexer search, quality profiles, grab pipeline, post-processing, and attention alerts.
...
3. **Grab** — `mediaGrabberGrab.ts` sends the chosen release to qBittorrent under the `hously-movies` or `hously-shows` category (constants from `apps/api/src/constants/libraryGrab.ts`).
