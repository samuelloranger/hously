...
`resolveHouslyInternalUrl()` (`apps/api/src/routes/integrations/qbittorrent/index.ts`) picks the URL qBittorrent uses to reach Hously, with this priority:

1. Explicit override from the admin
2. Docker DNS lookup of `hously` on `API_PORT` — works when qBittorrent (often in a VPN container via `network_mode: service:vpn`) shares the homelab network
