...
- **Push notifications** — Web Push (VAPID) via `web-push`. Keys load from `vapid_keys/` files or `VAPID_*` env vars (`apps/api/src/utils/webpush.ts`). User subscriptions are stored on `UserSubscription`. APNs is not currently wired in the API; the native iOS app in `../hously-ios` is a separate repo.
