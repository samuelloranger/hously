...
export const storagePaths = pgTable("storage_paths", {
	id: serial().primaryKey().notNull(),
	name: varchar().notNull(),
	...
	percentUsed: doublePrecision("percent_used"),
	lastCheckedAt: timestamp("last_checked_at", { mode: 'string' }),
	isAccessible: integer("is_accessible").notNull(),
	addedBy: integer("added_by").notNull(),
	createdAt: timestamp("created_at", { mode: 'string' }).notNull(),
}, (table) => [
	foreignKey({
			columns: [table.addedBy],
			foreignColumns: [users.id],
			name: "storage_paths_added_by_fkey"
		}),
	unique("storage_paths_path_key").on(table.path),
]);
...
export const services = pgTable("services", {
	...
	lastOfflineAt: timestamp("last_offline_at", { mode: 'string' }),
	lastDownNotificationSent: timestamp("last_down_notification_sent", { mode: 'string' }),
	lastUpNotificationSent: timestamp("last_up_notification_sent", { mode: 'string' }),
	addedBy: integer("added_by").notNull(),
	createdAt: timestamp("created_at", { mode: 'string' }).notNull(),
}, (table) => [
	index("ix_services_is_online").using("btree", table.isOnline.asc().nullsLast().op("bool_ops")),
	foreignKey({
			columns: [table.addedBy],
			foreignColumns: [users.id],
			name: "services_added_by_fkey"
		}),
]);
