...
describe('Notifications API', () => {
    ...
    it('should return VAPID public key or 503 without auth', async () => {
        if (!hasDb) return;
        const response = await app.handle(new Request('http://localhost/api/notifications/vapid-public-key'))
    ...
    it('should return 401 when unauthenticated on GET /unread-count', async () => {
        if (!hasDb) return;
        const response = await app.handle(new Request('http://localhost/api/notifications/unread-count'))
        expect(response.status).toBe(401)
    })

    it('should return 401 when unauthenticated on PUT /read-all', async () => {
        if (!hasDb) return;
        const response = await app.handle(new Request('http://localhost/api/notifications/read-all', {
            method: 'PUT'
        }))
        expect(response.status).toBe(401)
    })
    ...
    it('should return subscribed devices list when authenticated', async () => {
        if (!hasDb) return;
        const response = await app.handle(new Request('http://localhost/api/notifications/devices', {
            headers: { 'Cookie': cookies }
        }))
        expect(response.status).toBe(200)
        const json = await response.json() as any
