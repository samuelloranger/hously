...
describe('Board Tasks API', () => {
    ...
    it('creates a task with backlog status', async () => {
        if (!hasDb) return;
        const { res, json } = await createTask({ title: 'Backlog task', status: 'backlog' })
        expect(res.status).toBe(200)
    ...
    it('creates a task with urgent priority', async () => {
        if (!hasDb) return;
        const { res, json } = await createTask({ title: 'Urgent task', priority: 'urgent' })
        expect(res.status).toBe(200)
    ...
    it('creates a task with assignee_id', async () => {
        if (!hasDb) return;
        const { res, json } = await createTask({ title: 'Assigned task', assignee_id: userId })
        expect(res.status).toBe(200)
    ...
    it('updates a task status', async () => {
        if (!hasDb) return;
        const { json: created } = await createTask({ title: 'Status update test', status: 'todo' })
        const id = created.task.id
    ...
    it('updates task priority', async () => {
        if (!hasDb) return;
        const { json: created } = await createTask({ title: 'Priority update', priority: 'low' })
        const id = created.task.id
    ...
    it('clears task due date with null', async () => {
        if (!hasDb) return;
        const { json: created } = await createTask({ title: 'Clear date', due_date: '2025-12-31' })
        const id = created.task.id
    ...
    it('updates task tags', async () => {
        if (!hasDb) return;
        const { json: created } = await createTask({ title: 'Tag update', tags: ['old'] })
        const id = created.task.id
    ...
    it('clears task assignee with null', async () => {
        if (!hasDb) return;
        const { json: created } = await createTask({ title: 'Clear assignee', assignee_id: userId })
        const id = created.task.id
