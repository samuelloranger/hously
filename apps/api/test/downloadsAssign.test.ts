...
mock.module("@hously/api/services/libraryFromTmdb", () => ({
...
mock.module("@hously/api/services/jellyfinLibraryRefresh", () => ({
  triggerJellyfinLibraryScan: async () => {},
}));

import { assignDownloadFromDisk } from "../src/services/downloadsAssign";
import { invalidateDownloadsScannerCache } from "../src/services/downloadsScanner";
...
describe("assignDownloadFromDisk validation", () => {
  ...
  beforeEach(async () => {
    root = await mkdtemp(join(tmpdir(), "hously-assign-"));
  ...
  it("returns 400 when file_path is outside a downloads root", async () => {
    const outsideDir = await mkdtemp(join(tmpdir(), "hously-outside-"));
