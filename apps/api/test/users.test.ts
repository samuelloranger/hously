...
describe('Users API', () => {
    ...
    it('should return 401 when unauthenticated on GET /me', async () => {
        if (!hasDb) return;
        const response = await app.handle(new Request('http://localhost/api/users/me'))
    ...
    it('should return 401 when unauthenticated on POST /me/password', async () => {
        if (!hasDb) return;
        const response = await app.handle(new Request('http://localhost/api/users/me/password', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
