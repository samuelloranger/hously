...
describe("Shopping API", () => {
  ...
  it("should toggle a shopping item completion", async () => {
    ...
    const response = await app.handle(
      new Request(`http://localhost/api/shopping/${id}/toggle`, {
        method: "POST",
        headers: { Cookie: cookies },
      }),
    );
  ...
  it("should update a shopping item", async () => {
    ...
    const response = await app.handle(
      new Request(`http://localhost/api/shopping/${id}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json", Cookie: cookies },
  ...
  it("should return 404 for non-existent item on toggle", async () => {
    if (!hasDb) return;
    const response = await app.handle(
      new Request("http://localhost/api/shopping/999999/toggle", {
        method: "POST",
        headers: { Cookie: cookies },
      }),
    );
  ...
  it("should clear completed items", async () => {
    if (!hasDb) return;
    const response = await app.handle(
      new Request("http://localhost/api/shopping/clear-completed", {
        method: "POST",
        headers: { Cookie: cookies },
      }),
    );
  ...
  it("should reorder shopping items", async () => {
    ...
    const response = await app.handle(
      new Request("http://localhost/api/shopping/reorder", {
        method: "POST",
        headers: { "Content-Type": "application/json", Cookie: cookies },
