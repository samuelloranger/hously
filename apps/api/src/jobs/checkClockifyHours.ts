...
import { CLOCKIFY_API_BASE, CLOCKIFY_PAGE_SIZE, parseDuration, getCurrentWeekRange, submitApprovalRequest } from '../services/clockify';

const JOB_ID = 'checkClockifyHours';
const JOB_NAME = 'Check Clockify weekly hours';
const TARGET_HOURS = 37.5;

export const checkClockifyHours = async (
  options?: { trigger?: 'cron' | 'manual' | 'queue' }
): Promise<void> => {
  ...
  const endLog = async (success: boolean, message?: string) => {
    await logActivity({
      type: 'cron_job_ended',
      payload: { job_id: JOB_ID, job_name: JOB_NAME, success, duration_ms: Date.now() - startedAt, trigger, message },
    });
  };

  try {
    const plugin = await prisma.plugin.findFirst({ where: { type: 'clockify' } });
    ...
    if (!config.api_key || !config.workspace_id || !config.user_id) {
      await logActivity({
        type: 'cron_job_skipped',
        payload: { job_id: JOB_ID, job_name: JOB_NAME, reason: 'invalid_config', trigger },
      });
    ...
    console.log(`[cron:clockify] checking hours for week ${start.toISOString()} – ${end.toISOString()}`);
    ...
    while (hasMore) {
      ...
      const isLastPage = res.headers.get('Last-Page') === 'true' || entries.length < CLOCKIFY_PAGE_SIZE;
      hasMore = !isLastPage;
      page++;
    }
    ...
    console.log(`[cron:clockify] ${totalHours.toFixed(2)}h logged for week ${weekLabel} (target: ${TARGET_HOURS}h)`);
    ...
    if (totalHours < TARGET_HOURS) {
      ...
      try {
        ...
        submitted = true;
        console.log(`[cron:clockify] approval request submitted for week ${weekLabel}`);
      } catch (error) {
        const message = error instanceof Error ? error.message : 'Unknown error';
        console.error(`[cron:clockify] failed to submit approval request: ${message}`);
      }
