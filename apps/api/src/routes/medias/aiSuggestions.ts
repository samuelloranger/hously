...
import { normalizeOllamaConfig, normalizeRadarrConfig, normalizeSonarrConfig } from '../../utils/plugins/normalizers';
import { badGateway, badRequest, serviceUnavailable } from '../../utils/errors';
import {
  type TmdbSearchItem,
  mapTmdbSearchItem,
  fetchRadarrTmdbIds,
...
function parseAiJson(raw: string): Array<{ tmdb_id: unknown; media_type: unknown; reason: unknown }> {
...
export const mediasAiSuggestionsRoutes = new Elysia({ prefix: '/api/medias' })
  .use(auth)
  .use(requireUser)
  .get('/ai/suggestions/config', async () => {
    const plugin = await prisma.plugin.findFirst({ where: { type: 'ollama' } });
    const config = plugin?.enabled ? normalizeOllamaConfig(plugin.config) : null;
    return {
  ...
  .post(
    '/ai/suggestions',
    async ({ body, set }) => {
      const ollamaPlugin = await prisma.plugin.findFirst({ where: { type: 'ollama' } });
      const ollamaConfig = ollamaPlugin?.enabled ? normalizeOllamaConfig(ollamaPlugin.config) : null;
      if (!ollamaConfig) {
      ...
      try {
        if (mediaType === 'movie') {
          pool = await fetchPopularPool(tmdbConfig.api_key, language, 'movie', 2);
      ...
      pool = pool.filter((x, i, a) => a.findIndex(y => y.tmdb_id === x.tmdb_id && y.media_type === x.media_type) === i);
      if (pool.length < 8) {
        return badRequest(set, 'Not enough TMDB candidates for suggestions');
      }

      const enrichedPool = await enrichWithArr(pool, radarrConfig, sonarrConfig);
      const candidates = toCandidates(enrichedPool);
      const prompt = (body.prompt ?? '').trim() || 'Pick a diverse, interesting mix worth watching.';
...
${JSON.stringify(candidates)}`;
      ...
      let picks: Array<{ tmdb_id: number; media_type: 'movie' | 'tv'; reason: string }> = [];
      try {
        ...
        for (const row of arr) {
          const id = typeof row.tmdb_id === 'number' ? row.tmdb_id : parseInt(String(row.tmdb_id), 10);
          const mt = row.media_type === 'tv' ? 'tv' : row.media_type === 'movie' ? 'movie' : null;
          const reason = typeof row.reason === 'string' ? row.reason.slice(0, 280) : '';
        ...
        return badGateway(set, 'Could not parse AI response. Try again or use a different model.');
      }

      picks = picks.slice(0, 12);
      if (picks.length === 0) {
        return badGateway(set, 'The model returned no valid picks from the list. Try again.');
      }

      const reasonByKey = new Map<string, string>(picks.map(p => [`${p.media_type}:${p.tmdb_id}`, p.reason]));
      const ordered: TmdbSearchItem[] = [];
      for (const p of picks) {
        const it = enrichedPool.find(x => x.tmdb_id === p.tmdb_id && x.media_type === p.media_type);
    ...
    },
    {
      body: t.Object({
        ...
        media_type: t.Union([t.Literal('movie'), t.Literal('tv'), t.Literal('both')]),
