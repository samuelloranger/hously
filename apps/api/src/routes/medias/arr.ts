...
export const mediasArrRoutes = new Elysia({ prefix: '/api/medias' })
  .use(auth)
  .use(requireUser)
  .post(
    ...
    async ({ user, set, params }) => {
      ...
      try {
        if (service === 'radarr') {
          ...
          if (!commandRes.ok) {
            return badGateway(set, `Radarr auto-search failed with status ${commandRes.status}`);
          }
        ...
        if (!commandRes.ok) {
          return badGateway(set, `Sonarr auto-search failed with status ${commandRes.status}`);
        }
    ...
    async ({ user, set, params, query }) => {
      ...
      try {
        if (service === 'radarr') {
          ...
          if (!releaseRes.ok) {
            return badGateway(set, `Radarr interactive search failed with status ${releaseRes.status}`);
          }
        ...
        if (!releaseRes.ok) {
          return badGateway(set, `Sonarr interactive search failed with status ${releaseRes.status}`);
        }
    ...
    async ({ user, set, params, body }) => {
      ...
      try {
        if (service === 'radarr') {
          ...
          if (!commandRes.ok) {
            return badGateway(set, `Radarr download release failed with status ${commandRes.status}`);
          }
        ...
        if (!commandRes.ok) {
          return badGateway(set, `Sonarr download release failed with status ${commandRes.status}`);
        }
    ...
    async ({ user, set, params, query }) => {
      ...
      try {
        if (service === 'radarr') {
          ...
          if (!deleteRes.ok) {
            return badGateway(set, `Radarr delete failed with status ${deleteRes.status}`);
          }
        ...
        if (!deleteRes.ok) {
          return badGateway(set, `Sonarr delete failed with status ${deleteRes.status}`);
        }
