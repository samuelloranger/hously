...
import { badGateway, badRequest, notFound, serverError } from '../../utils/errors';
import {
  ...
  .get(
    ...
    async ({ user, set, query }) => {
      ...
      if (searchQuery.length < 2) {
        return badRequest(set, 'Search query must be at least 2 characters long');
      }
      ...
      try {
        ...
        if (!response.ok) {
          return badGateway(set, `Prowlarr interactive search failed with status ${response.status}`);
        }
        ...
        return serverError(set, 'Failed to load Prowlarr interactive search releases');
      }
    },
    {
      query: t.Object({
  ...
  .post(
    ...
    async ({ user, set, body }) => {
      ...
      try {
        ...
        if (!releasePayload) {
          return notFound(set, 'Selected release is no longer available. Run the search again.');
        }
        ...
        if (!response.ok) {
          return badGateway(set, `Prowlarr download release failed with status ${response.status}`);
        }
