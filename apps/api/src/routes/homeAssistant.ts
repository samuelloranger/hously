...
import { haCallService, haGetStatesForEntities } from '../services/homeAssistant';
import {
...
function friendlyNameFromState(attributes: Record<string, unknown> | undefined, entityId: string): string {
  if (attributes && typeof attributes.friendly_name === 'string' && attributes.friendly_name.trim()) {
...
export const homeAssistantRoutes = new Elysia({ prefix: '/api/home-assistant' })
  ...
  .get('/widget', async ({ user: _user, set }) => {
    try {
      ...
      const states = await haGetStatesForEntities(cfg.base_url, cfg.access_token, cfg.enabled_entity_ids);
      if (!states.ok) {
        set.status = states.status >= 400 && states.status < 600 ? states.status : 502;
  ...
  .post(
    ...
    async ({ body, user: _user, set }) => {
      try {
        ...
        const result = await haCallService(cfg.base_url, cfg.access_token, domain, service, entityId);
        if (!result.ok) {
          set.status = result.status >= 400 && result.status < 600 ? result.status : 502;
    ...
    },
    {
      body: t.Object({
        entity_id: t.String(),
        action: t.Union([t.Literal('on'), t.Literal('off'), t.Literal('toggle')]),
      }),
    }
  );
