...
const QBITTORRENT_CONFIG_ERROR = 'qBittorrent plugin is disabled or not configured';
...
export function applyQbittorrentFetchStatus<T extends { connected: boolean }>(set: MutableStatus, result: T): T {
  if (!result.connected) {
    set.status = 502;
  }
  return result;
...
export function applyQbittorrentMutationStatus<T extends { connected: boolean; success: boolean }>(
  set: MutableStatus,
  result: T
): T {
...
export function validateQbittorrentUploadRequest(
  ...
  if (inputTorrents.length === 0) {
    console.warn(`${logPrefix} missing torrent files body.torrents_type=${typeof payload.torrents}`);
    return badRequest(set, 'Missing torrent files');
  }

  if (inputTorrents.length > 10) {
    console.warn(`${logPrefix} too many torrent files count=${inputTorrents.length}`);
