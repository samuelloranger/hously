...
const taskInclude = {
  ...
  blockingDependencies: {
    include: { blockedTask: { select: { id: true as const, title: true as const, status: true as const } } },
  },
  blockedDependencies: {
    include: { blockingTask: { select: { id: true as const, title: true as const, status: true as const } } },
  },
};
...
type TaskRow = {
  ...
  blockingDependencies: { id: number; blockedTaskId: number; blockedTask: { id: number; title: string; status: BoardTaskStatus } }[];
  blockedDependencies: { id: number; blockingTaskId: number; blockingTask: { id: number; title: string; status: BoardTaskStatus } }[];
};
...
async function wouldCreateCycle(blockingId: number, blockedId: number): Promise<boolean> {
...
export const boardTasksRoutes = new Elysia({ prefix: '/api/board-tasks' })
      ...
      try {
        ...
        const created = await prisma.boardTask.create({
          data: {
            ...
            tags: [],
            boardTags: tagIds.length > 0 ? { connect: tagIds.map((id: number) => ({ id })) } : undefined,
            createdBy: user!.id,
          },
          include: taskInclude,
        });
    ...
    async ({ params, body, set, user }) => {
      ...
      try {
        ...
        if (statusRaw !== undefined) {
          ...
          if (nextStatus !== existing.status) {
            ...
            activityInserts.push({
              ...
              userId: user!.id,
              type: 'status_change',
              metadata: { from: PRISMA_TO_API_STATUS[existing.status], to: PRISMA_TO_API_STATUS[nextStatus] },
            });
          }
        }
        ...
        if (priorityRaw !== undefined) {
          ...
          if (nextPriority !== existing.priority) {
            activityInserts.push({
              ...
              metadata: { from: PRISMA_TO_API_PRIORITY[existing.priority], to: PRISMA_TO_API_PRIORITY[nextPriority] },
            });
          }
        ...
        if (body.tag_ids !== undefined) {
          data.boardTags = {
            set: Array.isArray(body.tag_ids) ? body.tag_ids.map((id: number) => ({ id })) : [],
          };
        }
  ...
  .post(
    ...
    async ({ params, body, set }) => {
      ...
      if ((blocking_task_id === undefined) === (blocked_task_id === undefined)) {
        return badRequest(set, 'Provide exactly one of blocking_task_id or blocked_task_id');
      }
  ...
  .delete('/:id/dependencies/:dependencyId', async ({ params, set }) => {
    ...
    try {
      const dep = await prisma.boardTaskDependency.findUnique({ where: { id: dependencyId } });
      if (!dep) {
