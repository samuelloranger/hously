...
function validateFlareSolverrResponse(
  ...
  if (typeof s.userAgent !== "string" || !s.userAgent)
    throw new Error(
      `${trackerLabel} FlareSolverr response missing solution.userAgent`,
    );
  if (!Array.isArray(s.cookies))
    throw new Error(
      `${trackerLabel} FlareSolverr response missing solution.cookies array`,
    );
  const cookies: FlareSolverrCookie[] = (s.cookies as unknown[]).map((c, i) => {
    if (!c || typeof c !== "object" || Array.isArray(c))
      throw new Error(
        `${trackerLabel} FlareSolverr cookie[${i}] is not an object`,
      );
    const cookie = c as Record<string, unknown>;
    if (typeof cookie.name !== "string")
      throw new Error(`${trackerLabel} FlareSolverr cookie[${i}].name missing`);
    if (typeof cookie.value !== "string")
      throw new Error(
        `${trackerLabel} FlareSolverr cookie[${i}].value missing`,
      );
    if (typeof cookie.domain !== "string")
      throw new Error(
        `${trackerLabel} FlareSolverr cookie[${i}].domain missing`,
      );
    return {
      name: cookie.name,
      value: cookie.value,
...
export const fetchTrackerStats = async (
  ...
  try {
    ...
    if (!scraper) {
      await logActivity({
        ...
        payload: {
          job_id: jobId,
          job_name: jobName,
          reason: "scraper_not_implemented",
          trigger,
        },
      });
      return;
    ...
    const loginConfig = config.password
    ...
    if (scraper.needsFlaresolverr !== false) {
      if (!config.flaresolverr_url) {
        ...
        await logActivity({
          ...
          payload: {
            job_id: jobId,
            job_name: jobName,
            reason: "missing_flaresolverr_url",
            trigger,
          },
        });
        return;
      ...
      const fsJson = validateFlareSolverrResponse(
        fsRaw,
        trackerName(trackerType),
      );
      if (!fsRes.ok) {
        throw new Error(
          `${trackerName(trackerType)} FlareSolverr request failed`,
        );
      }
  ...
  try {
    for (const trackerType of TRACKER_ORDER) {
      try {
        ...
        console.error(
          `[cron:${trackerType}] fetch failed in batch run:`,
          message,
        );
      }
    }
