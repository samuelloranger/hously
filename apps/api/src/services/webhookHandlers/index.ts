...
// Beszel webhook handler
// Beszel sends alerts via Shoutrrr generic webhook.
// Configure in Beszel as: generic+https://<hously>/api/webhooks/beszel?token=<token>&template=json&titlekey=title
// Payload: { "title": "...", "message": "..." }
// Without template=json, Shoutrrr sends a plain-text body: "title\n\nmessage\n\nlink"
const handleBeszelWebhook: WebhookHandler = payload => {
  ...
  // When no separate title (plain text mode), first line is the title
  ...
  if (/\bis down\b/.test(t)) {
    ...
    eventType = 'StatusUp';
    system_name = (displayTitle || '').replace(/\s+is\s+back\s+up.*/i, '').trim();
  } else if (/above threshold/.test(t)) {
    eventType = 'AlertTriggered';
    // "homeserver CPU above threshold" → system_name="homeserver", alert_name="CPU"
    const words = (displayTitle || '').replace(/\s+above\s+threshold.*/i, '').trim().split(/\s+/);
    alert_name = words.pop() ?? '';
    system_name = words.join(' ');
  } else if (/below threshold/.test(t)) {
    eventType = 'AlertResolved';
    const words = (displayTitle || '').replace(/\s+below\s+threshold.*/i, '').trim().split(/\s+/);
    alert_name = words.pop() ?? '';
    system_name = words.join(' ');
  ...
  return {
    event_type: eventType,
    template_variables: ensureStrings({ title: displayTitle || rawBody, message, system_name, alert_name }),
    original_payload: payload,
  };
};
