...
const TRACKER_TAG_RULES: Array<{ matcher: (host: string) => boolean; tag: string; label: string }> = [
  { matcher: host => host.includes('c411'), tag: 'c411', label: 'C411' },
  { matcher: host => host.includes('torr9'), tag: 'torr9', label: 'Torr9' },
  { matcher: host => host.includes('la-cale') || host.includes('lacale'), tag: 'La Cale', label: 'La Cale' },
];
...
export async function enrichArrWebhookNotification(
  ...
  try {
    const trackersResult = await fetchQbittorrentTorrentTrackers(config, true, hash);
