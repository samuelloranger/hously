...
const C411_TOPBAR = 'div.hidden.lg\\:flex.items-center.h-9.gap-2.px-3.text-xs.font-medium.rounded-lg';
...
export const loginToC411 = async (page: Page, config: TrackerPluginConfig): Promise<void> => {
  ...
  try {
    ...
    const loginFormStillVisible = await loginForm.isVisible().catch(() => false);
    throw new Error(
