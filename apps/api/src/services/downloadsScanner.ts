...
import { remapPath } from "@hously/api/utils/medias/mediainfoScanner";
...
/** Library inode index for hardlink detection. */
export async function buildLibraryInodeKeySet(): Promise<Set<string>> {
...
export async function scanDownloads(
  ...
  const inodeKeySet = await buildLibraryInodeKeySet();
