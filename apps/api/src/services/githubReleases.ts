...
function cacheKey(kind: "releases" | "sync-state", repoFullName: string) {
  return `hously:github-releases:${kind}:v1:${encodeURIComponent(repoFullName)}`;
}
...
async function notifyAdminsForNewReleases(
  ...
  const title =
    releasesAheadOfCurrentVersion.length === 1
      ? `New Hously release: ${releasesAheadOfCurrentVersion[0].tag_name}`
      : `${releasesAheadOfCurrentVersion.length} new Hously releases`;
  const body =
    releasesAheadOfCurrentVersion.length === 1
