...
mock.module('@react-email/render', () => ({
  render: mock(async (_component: unknown, _opts?: unknown) => '<html>email</html>'),
}));
...
describe('emailService', () => {
  ...
  describe('sendEmail', () => {
    ...
    it('returns true and calls sendMail when SMTP is configured', async () => {
      ...
      const { sendEmail } = await import('../services/emailService');
      const result = await sendEmail('to@example.com', 'Hello', '<p>hi</p>', 'hi');

      expect(result).toBe(true);
      expect(sendMailMock).toHaveBeenCalledTimes(1);
      const call = (sendMailMock.mock.calls[0] as unknown as [Record<string, unknown>])[0];
  ...
  describe('sendInvitationEmail', () => {
    it('returns true without sending when SMTP is not configured', async () => {
      ...
      const result = await sendInvitationEmail('user@example.com', 'token123', 'Alice');
    ...
    it('uses French subject for fr locale', async () => {
      ...
      expect(sendMailMock).toHaveBeenCalledTimes(1);
      const callFr = (sendMailMock.mock.calls[0] as unknown as [Record<string, unknown>])[0];
    ...
    it('uses English subject for en locale', async () => {
      ...
      expect(sendMailMock).toHaveBeenCalledTimes(1);
      const callEn = (sendMailMock.mock.calls[0] as unknown as [Record<string, unknown>])[0];
  ...
  describe('sendPasswordResetEmail', () => {
    ...
    it('uses French subject for fr locale', async () => {
      ...
      expect(sendMailMock).toHaveBeenCalledTimes(1);
      const callFrReset = (sendMailMock.mock.calls[0] as unknown as [Record<string, unknown>])[0];
      expect(callFrReset.subject).toBe('Réinitialiser votre mot de passe Hously');
    });
    ...
    it('uses English subject by default', async () => {
      ...
      expect(sendMailMock).toHaveBeenCalledTimes(1);
      const callEnReset = (sendMailMock.mock.calls[0] as unknown as [Record<string, unknown>])[0];
