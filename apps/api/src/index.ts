...
export const app = new Elysia()
  ...
  .use((app) => {
    if (serveStatic) {
      ...
      app
        // Serve pre-compressed .gz assets when client supports gzip (generated by vite-plugin-compression2).
        .get("/assets/*", async ({ request, params }) => {
          ...
          if (acceptsGzip && (ext === "js" || ext === "css")) {
            const gzFile = Bun.file(`./public/assets/${filePath}.gz`);
          ...
          const file = Bun.file(`./public/assets/${filePath}`);
        ...
        .use(
          staticPlugin({
            assets: "./public",
            prefix: "/",
            // assets/* is handled above with gzip support; html served below with bootstrap injection
            ignorePatterns: [/\.html$/, /^assets[/\\]/],
          }),
        )
        .get("*", async ({ request }) => {
