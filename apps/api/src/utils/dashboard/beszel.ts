  ...
  async (): Promise<DashboardBeszelSummaryResponse> => {
    ...
    try {
      ...
      if (!authRes.ok) {
        return {
          ...buildBeszelDisabledSummary(
            `Beszel authentication failed with status ${authRes.status}`,
          ),
          enabled: true,
        };
      }
      ...
      if (!authData.token) {
        return {
          ...buildBeszelDisabledSummary(
            "Beszel authentication response missing token",
          ),
          enabled: true,
        };
      }
      ...
      const systemsUrl = new URL(
        "/api/collections/systems/records?perPage=1&sort=created",
        config.website_url,
      );
      ...
      if (!systemsRes.ok) {
        return {
          ...buildBeszelDisabledSummary(
            `Beszel systems request failed with status ${systemsRes.status}`,
          ),
          enabled: true,
        };
      }
      ...
      const systemsData =
        (await systemsRes.json()) as PocketBaseList<BeszelSystemRecord>;
      ...
      if (!statsRes.ok) {
        return {
          ...buildBeszelDisabledSummary(
            `Beszel stats request failed with status ${statsRes.status}`,
          ),
          enabled: true,
        };
      }
      ...
      const statsData =
        (await statsRes.json()) as PocketBaseList<BeszelStatsRecord>;
      if (!statsData.items.length) {
        return {
          ...buildBeszelDisabledSummary(
            "No stats available yet for this system",
          ),
          enabled: true,
        };
      }
      ...
      try {
        ...
        if (smartRes.ok) {
          const smartData = (await smartRes.json()) as {
      ...
      const rootModel =
        Object.entries(deviceModelMap).find(
          ([dev]) => !efsMountDevices.has(dev),
        )?.[1] ?? null;

      // RAM: stored in GiB, convert to MiB
      const ramUsedMib =
        s.mu != null ? Math.round(s.mu * 1024 * 10) / 10 : null;
      const ramTotalMib = s.m != null ? Math.round(s.m * 1024 * 10) / 10 : null;

      // Root disk
      const mainDiskPct =
        s.dp ?? (s.d > 0 ? Math.round((s.du / s.d) * 1000) / 10 : 0);
      ...
      const networkIn =
        s.b?.[0] != null ? Math.round(((s.b[0] * 8) / 1000) * 10) / 10 : null;
      const networkOut =
        s.b?.[1] != null ? Math.round(((s.b[1] * 8) / 1000) * 10) / 10 : null;
