...
const buildDisabledResponse = (
  error?: string,
): DashboardHackerNewsResponse => ({
  enabled: false,
  stories: [],
  feed_type: "top",
...
export const fetchHackerNewsStories =
  async (): Promise<DashboardHackerNewsResponse> => {
    ...
    if (!config) {
      return {
        ...buildDisabledResponse(
          "Hacker News plugin is enabled but not configured",
        ),
        enabled: true,
      };
    }
    ...
    try {
      ...
      if (!idsResponse.ok) {
        return {
          ...buildDisabledResponse(
            `HN API returned status ${idsResponse.status}`,
          ),
          enabled: true,
        };
      }
