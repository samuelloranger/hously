...
export const normalizeWeatherAddress = (address: string): string => address.trim().replace(/\s+/g, ' ').toLowerCase();
...
const formatLocationName = (result?: NominatimResult): string => {
  const area = result?.address?.city || result?.address?.town || result?.address?.village || result?.address?.county;
