...
export const fetchAdguardSummary = async (): Promise<DashboardAdguardSummaryResponse> => {
  ...
  if (!config) {
    return {
      ...buildAdguardDisabledSummary('AdGuard Home plugin is enabled but not configured'),
      enabled: true,
    };
  }
  ...
  try {
    ...
    if (!statusResponse.ok) {
      return {
        ...buildAdguardDisabledSummary(`AdGuard status request failed with status ${statusResponse.status}`),
        enabled: true,
      };
    }
    ...
    if (!statsResponse.ok) {
      return {
        ...buildAdguardDisabledSummary(`AdGuard stats request failed with status ${statsResponse.status}`),
        enabled: true,
      };
    }

    const [statusPayload, statsPayload] = (await Promise.all([statusResponse.json(), statsResponse.json()])) as [
      unknown,
      unknown,
    ];
    ...
    const dnsQueries = Math.max(0, Math.trunc(toNumberOrNull(stats.num_dns_queries) ?? 0));
    const blockedQueries = Math.max(0, Math.trunc(toNumberOrNull(stats.num_blocked_filtering) ?? 0));
    const blockedRatio = dnsQueries > 0 ? Math.round((blockedQueries / dnsQueries) * 1000) / 10 : null;
    ...
    return {
      ...
      summary: {
        ...
        safebrowsing_blocked: Math.max(0, Math.trunc(toNumberOrNull(stats.num_replaced_safebrowsing) ?? 0)),
        safesearch_rewritten: Math.max(0, Math.trunc(toNumberOrNull(stats.num_replaced_safesearch) ?? 0)),
        parental_blocked: Math.max(0, Math.trunc(toNumberOrNull(stats.num_replaced_parental) ?? 0)),
      },
