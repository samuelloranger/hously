...
export type QbittorrentSortKey =
  | "name"
  | "ratio"
  | "added_on"
  | "size"
...
const QBITTORRENT_PAUSED_STATES = new Set([
  "pauseddl",
  "pausedup",
  "stopped",
  "stoppeddl",
...
const QBITTORRENT_CHECKING_STATUS = {
  ...
  dot: "bg-cyan-400",
  badge:
    "text-cyan-700 bg-cyan-50 dark:text-cyan-400 dark:bg-cyan-400/10 border-cyan-200 dark:border-cyan-500/30",
  pulse: true,
} as const;
...
> = {
  metadl: {
    ...
    dot: "bg-teal-400",
    badge:
      "text-teal-700 bg-teal-50 dark:text-teal-400 dark:bg-teal-400/10 border-teal-200 dark:border-teal-500/30",
    pulse: true,
  },
  downloading: {
    ...
    dot: "bg-sky-400",
    badge:
      "text-sky-700 bg-sky-50 dark:text-sky-400 dark:bg-sky-400/10 border-sky-200 dark:border-sky-500/30",
    pulse: true,
  },
  uploading: {
  ...
  error: {
    ...
    dot: "bg-red-400",
    badge:
      "text-red-700 bg-red-50 dark:text-red-400 dark:bg-red-400/10 border-red-200 dark:border-red-500/30",
    pulse: false,
  },
  missingfiles: {
...
const QBITTORRENT_PROGRESS_GRADIENTS: Record<string, string> = {
  ...
  completed:
    "bg-gradient-to-r from-emerald-500 to-green-500 dark:from-emerald-400 dark:to-green-400",
  error:
    "bg-gradient-to-r from-red-500 to-rose-500 dark:from-red-400 dark:to-rose-400",
  missingfiles:
    "bg-gradient-to-r from-red-500 to-rose-500 dark:from-red-400 dark:to-rose-400",
  moving:
    "bg-gradient-to-r from-cyan-500 to-sky-500 dark:from-cyan-400 dark:to-sky-400",
};
...
function getQbittorrentStateFilter(state: string): QbittorrentStateFilter {
  ...
  if (normalizedState === "metadl" || normalizedState === "downloading")
    return "downloading";
...
function isQbittorrentActivelyUploading(
  ...
  return (
    getQbittorrentStateFilter(torrent.state) === "seeding" &&
...
export function getQbittorrentStatusConfig(state: string) {
  ...
  return (
    QBITTORRENT_STATUS_CONFIG[normalizedState] ?? QBITTORRENT_UNKNOWN_STATUS
  );
}

export function getQbittorrentStatusDot(state: string): {
  dot: string;
  pulse: boolean;
} {
...
export function getQbittorrentProgressBarGradient(state: string): string {
  ...
  return (
    QBITTORRENT_PROGRESS_GRADIENTS[normalizedState] ??
    QBITTORRENT_PROGRESS_GRADIENT_DEFAULT
  );
}

export function getQbittorrentTrackerStatusLabelKey(
  status: number | null,
): string {
  switch (status) {
    case 0:
...
export function getQbittorrentTrackerStatusColor(
  status: number | null,
): string {
  switch (status) {
    case 2:
...
export function getUniqueQbittorrentCategories(
...
export function getUniqueQbittorrentTags(
  torrents: QbittorrentTorrentListItem[],
): string[] {
  return Array.from(
    new Set(torrents.flatMap((torrent) => torrent.tags).filter(Boolean)),
  );
}
...
export function filterAndSortQbittorrentTorrents(
  ...
  if (options.stateFilter !== "all") {
    result = result.filter((torrent) =>
      matchesQbittorrentStateFilter(torrent, options.stateFilter),
    );
  }
  ...
  if (options.selectedTags.length > 0) {
    result = result.filter((torrent) =>
      options.selectedTags.some((tag) => torrent.tags.includes(tag)),
    );
  }
  ...
  return [...result].sort((left, right) => {
    ...
    if (options.sortBy === "name")
      ...
      comparison = (left.ratio ?? -1) - (right.ratio ?? -1);
    else if (options.sortBy === "added_on")
      comparison = (left.added_on ?? "").localeCompare(right.added_on ?? "");
...
export function buildQbittorrentTorrentSearchParams(params?: {
  ...
  if (typeof params?.reverse === "boolean")
    search.set("reverse", params.reverse ? "true" : "false");
...
export function createQbittorrentUploadFormData(input: {
  ...
  const files = Array.isArray(input.torrents)
    ? input.torrents
    : [input.torrents];
...
export function toggleQbittorrentTagSelection(
  selectedTags: string[],
  tag: string,
): string[] {
...
export function toOptionalQbittorrentTags(
  tags: string[],
): string[] | undefined {
...
export function mergeQbittorrentFiles(
  existingFiles: File[],
  incomingFiles: ArrayLike<File>,
): File[] {
  const knownFiles = new Set(
    existingFiles.map((file) => getQbittorrentFileKey(file)),
  );
  const nextFiles = Array.from<File>(incomingFiles).filter(
    (file) => !knownFiles.has(getQbittorrentFileKey(file)),
  );
...
export function getQbittorrentStreamSnapshot(
  ...
  const torrent =
    listData?.torrents.find((item) => item.id === torrentHash) ?? null;
...
export function parseQbittorrentRid(
