...
import {
  ...
  type HomeAssistantDiscoverEntity,
} from '@hously/shared';
...
function HomeAssistantPluginSectionImpl({
  ...
  const [selectedIds, setSelectedIds] = useState<Set<string>>(
    () => new Set(data?.plugin?.enabled_entity_ids ?? [])
  );
  ...
  }, [data?.plugin?.base_url, data?.plugin?.enabled, data?.plugin?.enabled_entity_ids]);
  ...
  return (
    ...
    >
      <div className="space-y-4">
        ...
        <p className="text-xs text-neutral-500 dark:text-neutral-400">{t('settings.plugins.homeAssistant.devicesHint')}</p>
