...
import { useExportData, useImportData } from "../../features/admin/hooks";
...
export function DataExportTab() {
  ...
  const {
    ...
    mutateAsync: triggerExport,
  } = useExportData();

  const { mutateAsync: triggerImport, isPending: isImporting } =
    useImportData();
  ...
  const handleImport = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      toast.error(t("settings.dataExport.importError"));
      toast.error(
        (error as any)?.message || t("settings.dataExport.importError"),
      );
      // Reset file input
      if (fileInputRef.current) {
        fileInputRef.current.value = "";
            ...
            <div>
              ...
              >
                {isImporting
                  ...
                  : t("settings.dataExport.importButton")}
              </label>
            </div>
          </div>
        </div>
