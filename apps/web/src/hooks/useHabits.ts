...
const syncHabitStatus = (previousHabits: HabitsResponse | undefined, id: number, response: HabitStatusResponse) =>
...
export const useCompleteHabit = () => {
  ...
  return useMutation({
    ...
    onSuccess: (response, id) => {
      queryClient.setQueryData<HabitsResponse>(queryKeys.habits.list(), current =>
        syncHabitStatus(current, id, response)
      );
    },
    onSettled: () => {
      queryClient.invalidateQueries({ queryKey: queryKeys.habits.all });
