...
export function TimePicker({ hours, minutes, onHoursChange, onMinutesChange, className }: TimePickerProps) {
  ...
  const minutesList = Array.from({ length: 60 }, (_, i) => i).filter(m => m % 15 === 0);
  ...
  useEffect(() => {
    if (isOpen) {
      setTimeout(() => {
        if (hoursRef.current) {
          const selectedHourButton = hoursRef.current.querySelector(`[data-hour="${hours}"]`) as HTMLElement;
          if (selectedHourButton) {
            selectedHourButton.scrollIntoView({ block: 'center' });
          }
        }
        if (minutesRef.current) {
          const selectedMinuteButton = minutesRef.current.querySelector(`[data-minute="${minutes}"]`) as HTMLElement;
  ...
  return (
    <Popover open={isOpen} onOpenChange={setIsOpen}>
      <PopoverTrigger asChild>
        <Button variant="outline" className={cn('w-[140px] justify-start text-left font-normal', className)}>
      ...
      <PopoverContent className="w-auto p-0" align="start">
        <div className="flex p-2">
          ...
          <div className="flex flex-col items-center border-r border-neutral-200 dark:border-neutral-700 pr-2">
            <div className="text-xs font-medium text-neutral-500 dark:text-neutral-400 mb-1 px-2">Hours</div>
            <div
              ref={hoursRef}
          ...
          <div className="flex flex-col items-center pl-2">
            <div className="text-xs font-medium text-neutral-500 dark:text-neutral-400 mb-1 px-2">Minutes</div>
            <div
              ref={minutesRef}
              className="max-h-[200px] overflow-y-auto scroll-smooth [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]"
            >
