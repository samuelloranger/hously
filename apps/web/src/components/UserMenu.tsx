...
export function UserMenu({ user, onLogout }: UserMenuProps) {
  ...
  const toggleLanguage = () => {
    ...
    if (user) {
      updateProfile.mutate({ locale: nextLanguage.code }, { onError: error => {
        ...
        >
          {user.avatar_url ? (
            <img
              ...
              className="hidden xl:block mr-2 h-7 w-7 rounded-full object-cover border border-neutral-300 dark:border-neutral-600"
            />
          ) : (
        ...
        >
              ...
              return (
                <Link
                  ...
                  className={`block w-full text-left px-2 py-2 text-sm text-neutral-700 dark:text-neutral-300 hover:bg-neutral-100 dark:hover:bg-neutral-700 rounded-md ${
                    isActive ? 'text-primary-600 dark:text-primary-400 bg-neutral-50 dark:bg-neutral-700' : ''
                  }`}
                >
          ...
          <div className="px-4 py-2 border-b border-neutral-200 dark:border-neutral-700">
            <div className="flex items-center justify-between mb-2">
              <span className="text-xs font-medium text-neutral-500 dark:text-neutral-400">{t('common.language')}</span>
              <button
            ...
            >
              <span className="text-xs font-medium text-neutral-500 dark:text-neutral-400">{t('common.theme')}</span>
              <span>{isDark ? '☀️' : '🌙'}</span>
            </button>
          </div>
          <Link
