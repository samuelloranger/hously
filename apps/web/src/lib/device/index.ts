...
function detectOS(userAgent: string): { name: string; version: string | null } {
  ...
  if (userAgent.includes('Windows')) {
    if (userAgent.includes('Windows NT 10.0')) return { name: 'Windows', version: '10/11' };
...
export function getDeviceInfo(): DeviceInfo {
  ...
  const deviceName = (navigator as any).deviceMemory ? `${platform} (${(navigator as any).deviceMemory}GB)` : null;
