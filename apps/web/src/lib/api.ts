  ...
  if (isOffline && isMutation) {
    try {
      ...
      const errorMessage =
        ...
        queueError.message.includes("IndexedDB unavailable")
...
export const api = {
  ...
  async getNotificationDevices(): Promise<{
    ...
    return fetchApi<{ devices: Array<NotificationDevice> }>(
  ...
  async getCustomEvents(
    ...
    if (month) params.append("month", month.toString());
    const queryString = params.toString();
    return fetchApi<CustomEventsResponse>(
      `/api/custom-events${queryString ? `?${queryString}` : ""}`,
    );
  },

  async createCustomEvent(
    data: CreateCustomEventRequest,
  ): Promise<CustomEvent> {
  ...
  async deleteCustomEvent(
    ...
    return fetchApi<ApiResult<{ success: boolean }>>(
      `/api/custom-events/${eventId}`,
      {
        method: "DELETE",
      },
    );
