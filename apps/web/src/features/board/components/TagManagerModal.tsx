...
import { useBoardTags, useUpdateBoardTag, useDeleteBoardTag } from '@/hooks/useBoardTags';
...
export function TagManagerModal({ isOpen, onClose }: TagManagerModalProps) {
  ...
  const handleDelete = (id: number) => {
    deleteTag.mutate(
      { id, data: mergeIntoId ? { merge_into_id: Number(mergeIntoId) } : undefined },
      {
        onSuccess: () => {
          setDeleteConfirmId(null);
          setMergeIntoId('');
  ...
  return (
    <Dialog isOpen={isOpen} onClose={onClose} title="Manage Tags" panelClassName="max-w-md">
      <div className="flex flex-col gap-1">
        {tags.length === 0 && (
          <p className="py-6 text-center text-sm text-neutral-400">No tags yet. Create tags from the task drawer.</p>
        )}
        ...
        {tags.map(tag => {
          ...
          if (isEditing) {
            return (
              <div key={tag.id} className="flex flex-col gap-2 rounded-lg border border-indigo-200 bg-indigo-50/50 p-3 dark:border-indigo-700/40 dark:bg-indigo-900/10">
                <input
                  autoFocus
                  value={editName}
                ...
                <div className="flex flex-wrap gap-1.5">
                  <button
                    ...
                    className={cn(
                      ...
                      editColor === null ? 'border-neutral-500' : 'border-transparent'
                    )}
                    title="No color"
                  />
                  {TAG_COLORS.map(c => (
                    <button
                      ...
                      className={cn(
                        ...
                        editColor === c ? 'border-neutral-800 dark:border-white' : 'border-transparent'
                      )}
          ...
          if (isDeleting) {
            ...
            return (
              <div key={tag.id} className="flex flex-col gap-2 rounded-lg border border-red-200 bg-red-50/50 p-3 dark:border-red-700/40 dark:bg-red-900/10">
                <p className="text-sm text-neutral-700 dark:text-neutral-200">
                  Delete <strong>&ldquo;{tag.name}&rdquo;</strong>
                  {tag.task_count > 0 && (
                    <span className="ml-1 text-neutral-500">({tag.task_count} task{tag.task_count !== 1 ? 's' : ''})</span>
                  )}?
                </p>
                {others.length > 0 && (
                  <div className="flex items-center gap-2">
                    ...
                    <select
                      value={mergeIntoId}
                      onChange={e => setMergeIntoId(e.target.value ? Number(e.target.value) : '')}
                    ...
                    >
                      ...
                      {others.map(t => (
                        <option key={t.id} value={t.id}>Merge into &ldquo;{t.name}&rdquo;</option>
                      ))}
                    </select>
                  </div>
                )}
                <div className="flex gap-2">
                  ...
                  <Button
                    size="sm"
                    variant="ghost"
                    onClick={() => { setDeleteConfirmId(null); setMergeIntoId(''); }}
                    className="h-7 px-3 text-xs"
                  >
                    Cancel
                  </Button>
          ...
          return (
            ...
            >
              ...
              <span className="text-[11px] text-neutral-400">{tag.task_count} task{tag.task_count !== 1 ? 's' : ''}</span>
