...
function DepList({
  ...
  return (
    <div>
      ...
      {items.length === 0 && (
        <p className="text-[12px] text-neutral-400 dark:text-neutral-500">None</p>
      )}
      {items.map(dep => (
        <div key={dep.id} className="flex items-center gap-2 py-1">
          <span className="font-mono text-[10px] text-neutral-400">{dep.slug}</span>
          <span className="flex-1 truncate text-[12px] text-neutral-700 dark:text-neutral-200">{dep.title}</span>
          {'is_resolved' in dep && !dep.is_resolved && (
            <Lock className="h-3 w-3 shrink-0 text-orange-500" aria-label="Blocking" />
          )}
          <button
    ...
    ? allTasks
        .filter(
          t =>
            ...
            (t.slug.toLowerCase().includes(lower) || t.title.toLowerCase().includes(lower))
        )
        .slice(0, 6)
    : [];
      ...
      {open && matches.length > 0 && (
        <div className="absolute left-0 top-full z-50 mt-1 w-full overflow-hidden rounded-lg border border-neutral-200/80 bg-white shadow-lg dark:border-neutral-700/60 dark:bg-neutral-800">
          {matches.map(t => (
            ...
            >
              <span className="font-mono text-[10px] text-neutral-400">{t.slug}</span>
              <span className="truncate text-[12px] text-neutral-700 dark:text-neutral-200">{t.title}</span>
            </button>
          ))}
        </div>
      )}
    ...
    <div className="flex flex-col gap-4">
      <div>
        ...
        <AddDepInput
          ...
          onAdd={targetId => addDep.mutate({ id: task.id, data: { blocked_task_id: targetId } })}
      ...
      <div className="border-t border-neutral-100 pt-4 dark:border-neutral-800">
        ...
        <AddDepInput
          ...
          onAdd={targetId => addDep.mutate({ id: task.id, data: { blocking_task_id: targetId } })}
          isPending={addDep.isPending}
        />
      </div>
    </div>
