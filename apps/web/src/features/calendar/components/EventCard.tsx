  ...
  const getEventText = (event: CalendarEvent) => {
    if (event.type === "custom_event") {
      ...
      if (sameDay(start, end)) {
        return `${formatTime(start, i18n.language)} - ${formatTime(
          end.toISOString(),
          i18n.language,
        )}`;
      }
      return `${formatDateTime(start, i18n.language)} - ${formatDateTime(
        end,
        i18n.language,
      )}`;
    }
    ...
    >
      <div className="flex items-start justify-between">
        ...
        {event.type === "custom_event" ? (
          ...
          <span className="capitalize text-xs font-medium opacity-75 ml-2">
            {event.type}
          </span>
        )}
      </div>
