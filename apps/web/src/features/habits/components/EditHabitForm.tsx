...
export const EditHabitForm: React.FC<EditHabitFormProps> = ({ habit, onSubmit, isLoading }) => {
  const { t } = useTranslation('common');
  const [selectedEmoji, setSelectedEmoji] = useState(habit.emoji);
  const [times, setTimes] = useState<string[]>(habit.schedules.map(s => s.time));
  const [isActive, setIsActive] = useState(habit.active);

  const { register, handleSubmit, formState: { errors } } = useForm<Omit<UpdateHabitRequest, 'emoji' | 'schedules' | 'active'>>({
    defaultValues: {
      name: habit.name,
  ...
  const handleFormSubmit = (data: Omit<UpdateHabitRequest, 'emoji' | 'schedules' | 'active'>) => {
    onSubmit({
      ...data,
      emoji: selectedEmoji,
      schedules: times,
