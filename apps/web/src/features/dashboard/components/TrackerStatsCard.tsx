...
export function TrackerStatsCard() {
  ...
  return (
    <section
      className="relative overflow-hidden rounded-3xl border border-purple-300/60 dark:border-purple-200/30 bg-gradient-to-br from-[#e2c9f7] via-[#a174d2] to-[#c7b7f3] dark:from-purple-900/70 dark:via-purple-900/60 dark:to-violet-900/60 p-4 shadow-xl"
      onMouseEnter={() => prefetchRoute('/settings', { tab: 'plugins' })}
      onTouchStart={() => prefetchRoute('/settings', { tab: 'plugins' })}
    >
      <div className="flex items-start justify-between gap-4">
        <div>
          <p className="text-[9px] uppercase tracking-[0.22em] text-purple-950/70 dark:text-purple-200/90">
            {t('dashboard.trackers.kicker')}
          </p>
          <h3 className="text-base md:text-lg font-bold text-purple-950 dark:text-purple-50">
            {t('dashboard.trackers.title')}
          </h3>
          <p className="text-[10px] text-purple-900/70 dark:text-purple-100/90 mt-1">
            {t('dashboard.trackers.subtitle')}
          </p>
        </div>
        <span className="rounded-full bg-black/15 dark:bg-black/25 px-3 py-1 text-[11px] font-medium text-purple-950 dark:text-purple-100">
          {anyConnected
              ...
              return (
                <article key={item.key} className="rounded-lg bg-black/10 dark:bg-black/20 p-2.5">
                  <div className="flex items-center justify-between gap-1 mb-1.5">
                    <p className="text-[11px] font-semibold text-purple-950 dark:text-white truncate">{item.label}</p>
                    <span className="text-[8px] shrink-0 text-purple-950/70 dark:text-purple-100/80">
                      {showUnavailable ? t('dashboard.trackers.disconnected') : t('dashboard.trackers.connectedSimple')}
                    </span>
                  </div>

                  {showUnavailable ? (
                    <p className="text-[9px] leading-tight text-purple-950/85 dark:text-purple-100/90">
                      {item.error || t('dashboard.trackers.notConnectedDescription')}
                    </p>
                  ) : (
                    <div className="flex flex-col gap-1">
                      <div className="flex items-baseline justify-between">
                        <p className="text-[9px] text-purple-950/75 dark:text-purple-200/80">
                          {t('dashboard.trackers.uploaded')}
                        </p>
                        <span className="text-[11px] font-semibold text-purple-950 dark:text-white leading-none">{formatGo(item.uploaded_go)}</span>
                      </div>
                      <div className="flex items-baseline justify-between">
                        <p className="text-[9px] text-purple-950/75 dark:text-purple-200/80">
                          {t('dashboard.trackers.downloaded')}
                        </p>
                        <span className="text-[11px] font-semibold text-purple-950 dark:text-white leading-none">{formatGo(item.downloaded_go)}</span>
                      </div>
                      <div className="flex items-baseline justify-between">
                        <p className="text-[9px] text-purple-950/75 dark:text-purple-200/80">
                          {t('dashboard.trackers.ratio')}
                        </p>
                        <span className="text-[11px] font-semibold text-purple-950 dark:text-white leading-none">{formatRatio(item.ratio)}</span>
                      </div>
                    </div>
                  )}

                  {updatedLabel ? (
                    <p className="text-[8px] text-purple-950/70 dark:text-purple-100/80 mt-1.5">
