    ...
    <section className="relative overflow-hidden rounded-3xl border border-rose-300/60 dark:border-rose-200/40 bg-gradient-to-br from-[#f9d2dd] via-[#f6b6c8] to-[#ffe0cd] dark:from-rose-700 dark:via-rose-700 dark:to-orange-700 p-6 shadow-xl">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <h3 className="text-2xl md:text-3xl font-bold text-rose-950 dark:text-rose-50">{t('dashboard.scrutiny.title')}</h3>
          <p className="text-sm text-rose-900/70 dark:text-rose-100/90 mt-1">{t('dashboard.scrutiny.subtitle')}</p>
        </div>
      ...
      {showNotConnected ? (
        <div className="mt-5 rounded-2xl border border-rose-500/40 dark:border-rose-300/40 bg-rose-100/55 dark:bg-rose-100/15 p-4 text-rose-950 dark:text-rose-100">
          ...
          <p className="text-sm text-rose-950/80 dark:text-rose-100/90 mt-1">{t('dashboard.scrutiny.notConnectedDescription')}</p>
        </div>
      ) : (
        <>
          <div className="mt-5 grid grid-cols-2 lg:grid-cols-4 gap-3">
            <div className="rounded-xl bg-black/10 dark:bg-black/20 p-3">
              ...
              <p className="text-xl font-semibold text-rose-950 dark:text-white">{data?.summary.total_drives ?? 0}</p>
            </div>
            <div className="rounded-xl bg-black/10 dark:bg-black/20 p-3">
              <p className="text-xs text-rose-950/75 dark:text-rose-200/80">{t('dashboard.scrutiny.healthyDrives')}</p>
              <p className="text-xl font-semibold text-rose-950 dark:text-white">{data?.summary.healthy_drives ?? 0}</p>
            </div>
            <div className="rounded-xl bg-black/10 dark:bg-black/20 p-3">
              <p className="text-xs text-rose-950/75 dark:text-rose-200/80">{t('dashboard.scrutiny.warningDrives')}</p>
              <p className="text-xl font-semibold text-rose-950 dark:text-white">{data?.summary.warning_drives ?? 0}</p>
            </div>
            <div className="rounded-xl bg-black/10 dark:bg-black/20 p-3">
              <p className="text-xs text-rose-950/75 dark:text-rose-200/80">{t('dashboard.scrutiny.hottestTemp')}</p>
              <p className="text-xl font-semibold text-rose-950 dark:text-white">{formatTemp(data?.summary.hottest_temp_c ?? null)}</p>
            </div>
          </div>
              ...
              {topDrives.map(drive => (
                <div key={drive.id} className="rounded-xl bg-black/10 dark:bg-black/20 p-3">
                  <div className="flex items-center justify-between gap-2">
                    ...
                    <p className="text-xs text-rose-900 dark:text-rose-100/90">{getStatusLabel(drive.device_status, t)}</p>
                  </div>
                  <div className="mt-1 grid grid-cols-2 md:grid-cols-4 gap-2 text-xs text-rose-900/90 dark:text-rose-100/90">
                    ...
                    <span className="truncate">{drive.serial_number || '--'}</span>
                  </div>
                </div>
              ))}
            </div>
