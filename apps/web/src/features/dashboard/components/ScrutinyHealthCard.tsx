  ...
  return (
    ...
    >
      <div className="flex items-start justify-between gap-4">
        <div>
          <p className="text-[9px] uppercase tracking-[0.22em] text-rose-950/70 dark:text-rose-200/90">
            {t('dashboard.scrutiny.kicker')}
          </p>
          <h3 className="text-base md:text-lg font-bold text-rose-950 dark:text-rose-50">
            {t('dashboard.scrutiny.title')}
          </h3>
          <p className="text-[10px] text-rose-900/70 dark:text-rose-100/90 mt-1">{t('dashboard.scrutiny.subtitle')}</p>
        </div>
        <span className="rounded-full bg-black/15 dark:bg-black/25 px-3 py-1 text-[11px] font-medium text-rose-950 dark:text-rose-100">
      ...
      {showNotConnected ? (
        <div className="mt-5 rounded-2xl border border-rose-500/40 dark:border-rose-300/40 bg-rose-100/55 dark:bg-rose-100/15 p-4 text-rose-950 dark:text-rose-100">
          ...
          <p className="text-xs text-rose-950/80 dark:text-rose-100/90 mt-1">
        ...
        <>
          <div className="mt-5 grid grid-cols-2 lg:grid-cols-4 gap-3">
            <div className="rounded-xl bg-black/10 dark:bg-black/20 p-3">
              <p className="text-[11px] text-rose-950/75 dark:text-rose-200/80">{t('dashboard.scrutiny.totalDrives')}</p>
              <p className="text-xs font-semibold text-rose-950 dark:text-white">{data?.summary.total_drives ?? 0}</p>
            </div>
            <div className="rounded-xl bg-black/10 dark:bg-black/20 p-3">
              <p className="text-[11px] text-rose-950/75 dark:text-rose-200/80">{t('dashboard.scrutiny.healthyDrives')}</p>
              <p className="text-xs font-semibold text-rose-950 dark:text-white">{data?.summary.healthy_drives ?? 0}</p>
            </div>
            <div className="rounded-xl bg-black/10 dark:bg-black/20 p-3">
              <p className="text-[11px] text-rose-950/75 dark:text-rose-200/80">{t('dashboard.scrutiny.warningDrives')}</p>
              <p className="text-xs font-semibold text-rose-950 dark:text-white">{data?.summary.warning_drives ?? 0}</p>
            </div>
            <div className="rounded-xl bg-black/10 dark:bg-black/20 p-3">
              <p className="text-[11px] text-rose-950/75 dark:text-rose-200/80">{t('dashboard.scrutiny.hottestTemp')}</p>
              <p className="text-xs font-semibold text-rose-950 dark:text-white">
          ...
          {topDrives.length > 0 ? (
            <Collapsible open={showDrives} onOpenChange={setShowDrives} className="mt-5">
              <CollapsibleTrigger asChild>
                <button
                  type="button"
                  className="inline-flex items-center rounded-full border border-rose-950/20 dark:border-rose-200/40 bg-black/10 dark:bg-black/20 px-3 py-1.5 text-[11px] font-medium text-rose-950 dark:text-rose-100 hover:bg-black/20 dark:hover:bg-black/30"
                >
                ...
                {topDrives.map(drive => (
                  <div key={drive.id} className="rounded-xl bg-black/10 dark:bg-black/20 p-3">
                    <div className="flex items-center justify-between gap-2">
                      <p className="truncate text-xs font-medium text-rose-950 dark:text-white">
                        {drive.model_name || t('dashboard.scrutiny.unknownDrive')}
                      </p>
                      <p className="text-[11px] text-rose-900 dark:text-rose-100/90">
                        {getStatusLabel(drive.device_status, t)}
                      </p>
                    </div>
                    <div className="mt-1 grid grid-cols-2 md:grid-cols-4 gap-2 text-[11px] text-rose-900/90 dark:text-rose-100/90">
            ...
            <div className="mt-5 rounded-2xl border border-rose-500/40 dark:border-rose-300/40 bg-rose-100/55 dark:bg-rose-100/15 p-4 text-xs text-rose-950 dark:text-rose-100">
              {t('dashboard.scrutiny.emptyTitle')}
            </div>
          )}

          {data?.error && <p className="mt-4 text-[11px] text-rose-950/85 dark:text-rose-100/90">{data.error}</p>}
        </>
      )}
    </section>
  );
