    ...
    >
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-[11px] text-emerald-900/70 dark:text-emerald-100/90 mt-1">{t('dashboard.netdata.subtitle')}</p>
        </div>
      ...
      {showNotConnected ? (
        ...
        <>
          ...
          {data?.error && <p className="mt-4 text-xs text-emerald-950/85 dark:text-emerald-100/90">{data.error}</p>}
        </>
      )}
    </section>
  );
