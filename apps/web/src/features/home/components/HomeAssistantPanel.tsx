...
import { useHomeAssistantControl, useHomeAssistantWidget } from '@hously/shared';
        ...
        {entities.length === 0 ? (
          ...
          entities.map(e => {
            ...
            return (
              ...
              >
                <Icon className="size-5 shrink-0 text-amber-600 dark:text-amber-400" strokeWidth={1.75} aria-hidden />
                <div className="min-w-0 flex-1">
                  <p className="text-sm font-medium text-zinc-900 dark:text-zinc-100 truncate">{e.friendly_name}</p>
                ...
                <span
                  className={cn(
                    ...
                    on ? 'text-emerald-600 dark:text-emerald-400' : 'text-zinc-500'
                  )}
                >
                  {on ? t('dashboard.homeAssistant.on') : t('dashboard.homeAssistant.off')}
                </span>
                <button
                  type="button"
                  disabled={busy}
