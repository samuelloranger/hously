...
import { useDashboardQbittorrentTorrents } from '@hously/shared';
...
vi.mock('@/features/torrents/AddTorrentPanel', () => ({
  AddTorrentPanel: () => <div>Add Torrent</div>,
}));

vi.mock('@/features/torrents/TorrentRow', () => ({
  ...
  return {
    ...
    countQbittorrentTorrentsByState: (items: typeof torrents) => {
      ...
      for (const torrent of items) {
        const stateFilter = torrent.state === 'pauseddl' ? 'paused' : torrent.state;
      ...
      [...items]
        .filter(torrent => {
          const matchesState =
            ...
            (options.stateFilter === 'paused' ? torrent.state === 'pauseddl' : torrent.state === options.stateFilter);
          const matchesCategory =
            options.selectedCategories.length === 0 || options.selectedCategories.includes(torrent.category);
          const matchesTag =
            options.selectedTags.length === 0 || options.selectedTags.some((tag: string) => torrent.tags.includes(tag));
          const matchesSearch = !options.search || torrent.name.toLowerCase().includes(options.search.toLowerCase());
        ...
        .sort((left, right) => {
          ...
          else if (options.sortBy === 'ratio') comparison = left.ratio - right.ratio;
    ...
    getUniqueQbittorrentCategories: (items: typeof torrents) => [...new Set(items.map(torrent => torrent.category))],
...
describe('TorrentsPage', () => {
  ...
  it('pushes state changes to router history', () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: /torrents\.filterPaused/i }));
