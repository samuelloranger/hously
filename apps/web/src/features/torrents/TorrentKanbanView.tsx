...
export function TorrentKanbanView({ torrents }: { torrents: QbittorrentTorrentListItem[] }) {
  ...
  return (
    <div className="overflow-x-auto pb-2 -mx-1">
      <div className="flex gap-3 px-1" style={{ minWidth: `${KANBAN_COLUMNS.length * 17}rem` }}>
        {KANBAN_COLUMNS.map(col => {
          ...
          return (
            <div
              key={col.id}
              className={cn(
                'w-64 flex-shrink-0 flex flex-col rounded-2xl border overflow-hidden',
                col.borderColor
              )}
            >
              {/* Column header */}
              <div className={cn('px-3 py-2.5 flex items-center justify-between border-b', col.headerBg, col.borderColor)}>
                ...
                <span className={cn('text-xs font-bold tabular-nums', col.countColor)}>
                  {items.length}
                </span>
              </div>
