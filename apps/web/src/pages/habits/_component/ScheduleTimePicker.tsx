    ...
    <div className="space-y-3">
      ...
      <div className="flex flex-wrap gap-2">
        {times.map((time, index) => (
          <div
            key={index}
            className="flex items-center gap-1 p-1 pr-2 rounded-lg bg-neutral-700 border border-neutral-700"
          >
            <input
              type="time"
              value={time}
        ...
        <button
          type="button"
          onClick={addTime}
          className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-dashed border-neutral-700 text-neutral-400 hover:border-neutral-600 hover:text-neutral-300 transition-all text-sm font-medium"
        >
          <Plus size={14} />
          {t("habits.addTime")}
        </button>
