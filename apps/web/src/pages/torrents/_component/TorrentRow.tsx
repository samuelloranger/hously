...
export function TorrentRow({
  ...
  const isSeedingState = /^(uploading|forcedup|stalledup)$/i.test(
    torrent.state,
  );
  const isUploading = torrent.upload_speed > 0 && isSeedingState;
  const eta = formatQbittorrentEta(torrent.eta_seconds);
  const relDate =
    formatRelativeTime(torrent.added_on, { addSuffix: true, locale }) ?? "";
    ...
    >
      <div className="flex items-start gap-3 py-4">
        ...
        <div className="pt-1 shrink-0">
          <span
            className={`block w-2 h-2 rounded-full ${dot} ${pulse ? "animate-pulse" : ""}`}
          />
        </div>
        ...
        <div className="min-w-0 flex-1">
          ...
          {/* Progress bar */}
          <div
            className={`mt-2 h-1 w-full rounded-full overflow-hidden ${barTrackClass}`}
          >
            <div
