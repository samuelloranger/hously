...
type KanbanColumnId = 'downloading' | 'seeding' | 'paused' | 'complete' | 'error';
...
function getKanbanColumn(state: string): KanbanColumnId {
  ...
  if (
    ...
    return 'downloading';
  if (s === 'uploading' || s === 'stalledup' || s === 'forcedup' || s === 'queuedup') return 'seeding';
...
export function TorrentKanbanView({ torrents }: { torrents: QbittorrentTorrentListItem[] }) {
  const { t } = useTranslation('common');

  const columnMap = new Map<KanbanColumnId, QbittorrentTorrentListItem[]>(KANBAN_COLUMNS.map(col => [col.id, []]));
  ...
  return (
    <div className="overflow-x-auto pb-4 -mx-1">
      <div className="flex gap-3 px-1 items-start" style={{ minWidth: `${KANBAN_COLUMNS.length * 17}rem` }}>
        {KANBAN_COLUMNS.map(col => {
          const items = columnMap.get(col.id) ?? [];
          return (
            <div key={col.id} className={cn('w-64 flex-shrink-0 rounded-2xl border overflow-hidden', col.borderColor)}>
              ...
              >
                <div className="flex items-center gap-2">
                  ...
                  <span className={cn('text-xs font-semibold', col.titleColor)}>{t(col.titleKey)}</span>
                </div>
                <span className={cn('text-xs font-bold tabular-nums', col.countColor)}>{items.length}</span>
              </div>

              {/* Cards — natural height, page scrolls */}
              <div className={cn('space-y-2 p-2', col.emptyBg)}>
                {items.length === 0 ? (
                  <p className="text-center text-[11px] text-neutral-300 dark:text-neutral-700 py-8 select-none">—</p>
                ) : (
                  items.map(torrent => <KanbanCard key={torrent.id} torrent={torrent} />)
                )}
              </div>
            </div>
          );
