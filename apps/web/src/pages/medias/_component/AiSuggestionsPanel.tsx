...
import { useAiMediaSuggestions, useAiMediaSuggestionsConfig } from '@/hooks/useMedias';
...
export function AiSuggestionsPanel({ onAdded }: { onAdded: () => void }) {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const msg = e instanceof HttpError ? String(e.message) : t('medias.explore.aiError');
      toast.error(msg);
    }
  };
    ...
    <section className="overflow-hidden rounded-2xl border border-white/[0.08] bg-neutral-950/60 ring-1 ring-white/[0.04]">
      <div className="border-b border-white/[0.06] bg-gradient-to-r from-indigo-950/40 to-transparent px-4 py-3.5 md:px-5">
        <div className="flex flex-wrap items-start gap-3">
          ...
          <div className="min-w-0 flex-1">
            <h2 className="text-sm font-semibold tracking-tight text-white">{t('medias.explore.aiTitle')}</h2>
            <p className="mt-0.5 text-[11px] leading-relaxed text-white/45">{t('medias.explore.aiSubtitle')}</p>
          </div>
        </div>
      </div>
      ...
      <div className="space-y-3 px-4 py-4 md:px-5 md:py-5">
            ...
            {(
              ...
              <button
                ...
                className={`rounded-md px-2.5 py-1 text-[11px] font-medium transition-colors disabled:cursor-not-allowed disabled:opacity-45 ${
                  mediaType === value ? 'bg-indigo-500/25 text-indigo-100' : 'text-white/45 hover:text-white/70'
                }`}
              >
        ...
        {modelLabel && items && items.length > 0 && (
          <p className="text-[10px] text-white/35">{t('medias.explore.aiModelLabel', { model: modelLabel })}</p>
        )}
