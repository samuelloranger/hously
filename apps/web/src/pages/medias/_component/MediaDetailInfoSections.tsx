    ...
    <div className="flex flex-col gap-5">
      {hasFacts && (
        <Section title={t('medias.detail.sections.facts')}>
          {showOriginalTitle && <FactLine label={t('medias.detail.originalTitle')} value={details.original_title} />}
          {details.original_language_label && (
            <FactLine
          ...
          {countries.length > 0 && (
            <FactLine label={t('medias.detail.countries')} value={countries.map(c => c.name).join(', ')} />
          )}
          {spoken.length > 0 && (
            <FactLine
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {companies.map(c => (
                  ...
                  >
                    {c.logo_url ? <img src={c.logo_url} alt="" className="h-5 w-5 shrink-0 object-contain" /> : null}
      ...
      {hasTv && (
        <Section title={t('medias.detail.sections.tv')}>
          ...
          {runtimes.length > 0 && (
            <FactLine label={t('medias.detail.episodeRuntimes')} value={runtimes.map(m => `${m} min`).join(', ')} />
          )}
          {details.next_episode_to_air && (
