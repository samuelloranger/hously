...
import {
  ...
  type TabKey,
} from "@/pages/medias/_component/ExploreCardDetailDialog";
...
export function MediasLibrary() {
  ...
  const [density, setDensity] = usePersistentState<MediaDensity>(
    "media-library-density",
    "comfortable",
  );
  ...
  const isNotConfigured =
    libraryData && !libraryData.radarr_enabled && !libraryData.sonarr_enabled;

  const filtered = useMemo(() => {
    const scopedItems = items.filter((item) => {
      if (scopeFilter === "all") return true;
      if (scopeFilter === "radarr" || scopeFilter === "sonarr")
        return item.service === scopeFilter;
      if (scopeFilter === "downloaded") return item.downloaded;
      if (scopeFilter === "missing")
        return !item.downloaded && !item.downloading;
      return true;
    });
  ...
  const showingEnd =
    filtered.length === 0 ? 0 : Math.min(pageEndIndex, filtered.length);
  ...
  useEffect(() => {
    if (searchParams.scrollToMedia && !isLoading && items.length > 0) {
      const element = document.getElementById(
        `media-${searchParams.scrollToMedia}`,
      );
      if (element) {
  ...
  const typeFilters = [
    {
      value: "all" as const,
      label: t("medias.filterAll"),
      count: items.length,
    },
    {
      value: "movie" as const,
      label: t("medias.filterMovies"),
      count: movieCount,
    },
    {
      value: "series" as const,
      label: t("medias.filterSeries"),
      count: seriesCount,
    },
  ];
  ...
  const scopeFilters = [
    {
      value: "all" as const,
      label: t("medias.scopeAll", "Everything"),
      count: items.length,
    },
    {
      value: "radarr" as const,
      label: "Radarr",
      count: items.filter((i) => i.service === "radarr").length,
    },
    {
      value: "sonarr" as const,
      label: "Sonarr",
      count: items.filter((i) => i.service === "sonarr").length,
    },
    {
      value: "downloaded" as const,
      label: t("medias.downloaded"),
      count: downloadedCount,
    },
    {
      value: "missing" as const,
      label: t("medias.missing"),
      count: missingCount,
    },
  ];
        ...
        <div className="px-3 pt-3 pb-2.5 flex items-center gap-2">
          ...
          >
            ...
            <option value="downloaded">
              {t("medias.sortOptions.downloaded")}
            </option>
            <option value="monitored">
              {t("medias.sortOptions.monitored")}
            </option>
          </select>
