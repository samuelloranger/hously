...
import { useArrManagementDetails, useMediaDelete, useMediaRefresh } from '@/hooks/useMedias';
...
export function ArrManagementPanel({ service, sourceId, title, isActive, onDeleted }: ArrManagementPanelProps) {
  ...
  const {
    ...
    isError,
    refetch,
  } = useArrManagementDetails({ service, source_id: sourceId }, { enabled: isActive });
  ...
  return (
    <div className="flex flex-col gap-4 pb-6">
      ...
      <div className="rounded-xl border border-neutral-200/80 bg-neutral-50/80 p-3 dark:border-neutral-700/60 dark:bg-neutral-900/40">
        ...
        {isError && <p className="text-sm text-amber-700 dark:text-amber-400">{t('medias.management.detailsError')}</p>}
            ...
            {info.service === 'radarr' && file && (
              <>
                ...
                <DetailRow
                  ...
                  value={[file.audio_codec, file.audio_channels].filter(Boolean).join(' · ') || null}
                />
      ...
      <p className="text-xs text-neutral-500 dark:text-neutral-500">{t('medias.management.releaseReminderHint')}</p>
      ...
      >
        <div className="flex flex-col gap-4 py-1">
          ...
          <div className="flex justify-end gap-2 pt-2">
            ...
            >
              {deleteMutation.isPending ? t('medias.management.deleting') : t('medias.management.confirmDelete')}
            </button>
          </div>
        </div>
      </Dialog>
