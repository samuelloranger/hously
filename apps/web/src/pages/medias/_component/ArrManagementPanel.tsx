...
function DetailRow({
  ...
  return (
    <div className="flex gap-3 text-sm">
      <span className="w-[38%] shrink-0 text-neutral-500 dark:text-neutral-500">
        {label}
      </span>
      <span
        className={cn(
...
export function ArrManagementPanel({
  service,
  sourceId,
  title,
  isActive,
  onDeleted,
}: ArrManagementPanelProps) {
  ...
  const handleRefresh = async () => {
    ...
    try {
      ...
      toast.success(
        t("medias.management.refreshSuccess", { service: serviceName }),
      );
      await refetch();
    } catch {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      toast.success(
        t("medias.management.deleteSuccess", { title, service: serviceName }),
      );
      setDeleteOpen(false);
      setConfirmText("");
      setRiskAck(false);
      ...
      toast.error(
        t("medias.management.deleteFailed", { service: serviceName }),
      );
    }
  };
    ...
    <div className="flex flex-col gap-4 pb-6">
      ...
      <div className="rounded-xl border border-neutral-200/80 bg-neutral-50/80 p-3 dark:border-neutral-700/60 dark:bg-neutral-900/40">
        ...
        {isError && (
          <p className="text-sm text-amber-700 dark:text-amber-400">
            {t("medias.management.detailsError")}
          </p>
        )}
        ...
        {info && !isLoading && (
          <div className="flex flex-col gap-2">
            <DetailRow
              label={t("medias.management.path")}
              value={info.path}
              mono
            />
            <DetailRow
              label={t("medias.management.rootFolder")}
              value={info.root_folder_path}
              mono
            />
            <DetailRow
              ...
              value={
                info.monitored
                  ? t("medias.management.yes")
                  : t("medias.management.no")
              }
            />
            <DetailRow
              label={t("medias.management.arrStatus")}
              value={info.arr_status}
            />
            <DetailRow
              label={t("medias.management.addedToArr")}
              value={formatShortDate(info.added)}
            />
            {info.genres.length > 0 && (
              <DetailRow
                label={t("medias.management.genres")}
                value={info.genres.join(", ")}
              />
            )}
            {info.service === "radarr" && (
              <DetailRow
                label={t("medias.management.studio")}
                value={info.studio}
              />
            )}
            {info.service === "sonarr" && (
              <>
                <DetailRow
                  label={t("medias.management.network")}
                  value={info.network}
                />
                <DetailRow
                  label={t("medias.management.seriesType")}
                  value={info.series_type}
                />
                <DetailRow
            ...
            {info.service === "radarr" && file && (
              <>
                ...
                <DetailRow
                  label={t("medias.management.filePath")}
                ...
                />
                <DetailRow
                  label={t("medias.management.quality")}
                  value={file.quality_label}
                />
                <DetailRow
                  label={t("medias.management.customFormatScore")}
                  value={
                    file.custom_format_score != null
                      ? String(file.custom_format_score)
                      : null
                  }
                />
                <DetailRow
                  label={t("medias.management.fileDateAdded")}
                  value={formatShortDate(file.date_added)}
                />
                {file.languages.length > 0 && (
                  <DetailRow
                    label={t("medias.management.languages")}
                    value={file.languages.join(", ")}
                  />
                )}
                <DetailRow
                  label={t("medias.management.video")}
                  value={file.video_codec}
                />
                <DetailRow
                  label={t("medias.management.resolution")}
                  value={file.media_resolution}
                />
                <DetailRow
                  label={t("medias.management.audio")}
                  value={
                    [file.audio_codec, file.audio_channels]
                      .filter(Boolean)
                      .join(" · ") || null
                  }
                />
                <DetailRow
                  label={t("medias.management.edition")}
                  value={file.edition}
                />
                <DetailRow
                  label={t("medias.management.releaseGroup")}
                  value={file.release_group}
                />
                <DetailRow
                  label={t("medias.management.sceneName")}
                  value={file.scene_name}
                  mono
                />
              </>
            )}
            ...
            {info.service === "radarr" && info.has_file && !file && (
              <p className="text-xs text-neutral-500">
                {t("medias.management.filePending")}
              </p>
            )}
          </div>
        )}
      ...
      >
        <div className="flex flex-col gap-4 py-1">
          <p className="text-sm text-neutral-600 dark:text-neutral-400">
            {t("medias.management.deleteWarning", {
              title,
              service: serviceName,
            })}
          </p>
          ...
          <label className="flex cursor-pointer items-start gap-2 text-sm text-neutral-700 dark:text-neutral-300">
            ...
            />
            <span>
              {t("medias.management.deleteAck", { service: serviceName })}
            </span>
          </label>
