...
export function DiscoverPanel({ onAdded }: { onAdded: () => void }) {
  ...
  function switchType(type: 'movie' | 'tv') {
    ...
    setGenreId(null);
    setPage(1);
    if (type === 'tv'    && (sortBy === 'revenue.desc' || sortBy === 'primary_release_date.desc')) setSortBy('popularity.desc');
    if (type === 'movie' && sortBy === 'first_air_date.desc') setSortBy('popularity.desc');
  }

  function toggleProvider(id: number) { setProviderId(p => (p === id ? null : id)); setPage(1); }
  function toggleGenre(id: number)    { setGenreId(p => (p === id ? null : id));    setPage(1); }
  function changeSort(value: string)  { setSortBy(value); setPage(1); }
  function toggleLanguage(code: string) { setOriginalLanguage(p => (p === code ? null : code)); setPage(1); }
  ...
  return (
    <section className="relative space-y-5">
      ...
      <div
        ...
        style={{ scrollbarWidth: 'none', margin: '0 -16px', padding: '4px 16px 8px' }}
      >
        {(providersData?.providers ?? []).slice(0, 10).map(provider => {
          ...
          return (
            ...
            >
              <img src={provider.logo_url} alt={provider.name} className="h-9 w-9 rounded-lg object-contain md:h-8 md:w-8" />
            </button>
          );
        })}
      </div>
        ...
        {genresData?.genres.map(genre => {
          ...
          return (
            <button
              ...
              style={active ? { color: colors.text, backgroundColor: colors.bg, borderColor: `${colors.text}50` } : undefined}
              className={[
      ...
      <div className="flex flex-wrap items-center gap-3">
        ...
        <div className="flex items-center gap-1.5">
          <span className="text-[10px] font-medium uppercase tracking-widest text-neutral-600">Lang</span>
