...
import { datetimeLocalToUTC, formatUsername, toDateTimeLocal, tomorrow, type ChoreUser } from '@hously/shared';
    ...
    createMutation.mutate(
      {
        ...
        description: data.description && data.description !== '<p></p>' ? data.description : null,
        reminder_enabled: data.reminder_enabled,
        reminder_datetime:
          data.reminder_enabled && data.reminder_datetime ? datetimeLocalToUTC(data.reminder_datetime) : undefined,
        image_path: imagePath,
        subscription_info: data.reminder_enabled && subscription ? subscription : undefined,
        recurrence_type: data.recurrence_enabled && data.recurrence_type ? data.recurrence_type : null,
        recurrence_interval_days:
          data.recurrence_enabled && data.recurrence_type === 'daily_interval'
            ? data.recurrence_interval_days
            : undefined,
        recurrence_weekday:
          data.recurrence_enabled && data.recurrence_type === 'weekly' ? data.recurrence_weekday : undefined,
      },
      {
        onSuccess: () => {
          reset();
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
          <label htmlFor="chore_name" className="block text-sm font-medium text-neutral-700 dark:text-neutral-300">
            {t('chores.choreName')}
          </label>
          <input
            type="text"
        ...
        <div>
          <label htmlFor="assigned_to" className="block text-sm font-medium text-neutral-700 dark:text-neutral-300">
            {t('chores.assignTo')}
          </label>
          <select
            id="assigned_to"
            ...
            {users.map(user => (
              <option key={user.id} value={user.id}>
                {user.first_name || user.last_name
                  ? formatUsername([user.first_name, user.last_name].filter(Boolean).join(' '))
                  : user.email}
              </option>
            ))}
          </select>
      ...
      <div className="border-t border-neutral-200 dark:border-neutral-700 pt-4">
        {!subscription && (
          <p className="text-xs text-yellow-600 dark:text-yellow-400 mb-4">{t('chores.noSubscriptionWarning')}</p>
        )}
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <p className="text-xs text-neutral-500 dark:text-neutral-400 mt-1">{t('chores.reminderDescription')}</p>
          </div>
        ...
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <p className="text-xs text-neutral-500 dark:text-neutral-400 mt-1">
              {t('chores.recurrenceDescription') || 'Créez automatiquement une nouvelle tâche selon un planning'}
            </p>
          </div>
          <label className="relative inline-flex items-center cursor-pointer">
            <input
            ...
            {recurrenceType === 'weekly' && (
              <div>
                ...
                >
                  ...
                  <option value={2}>{t('chores.wednesday') || 'Mercredi'}</option>
      ...
      <div>
        <label htmlFor="chore_image" className="block text-sm font-medium text-neutral-700 dark:text-neutral-300 mb-2">
          {t('chores.image')}
        </label>
        {imagePreview && (
        ...
        />
        <p className="mt-1 text-xs text-neutral-500 dark:text-neutral-400">{t('chores.imageHelp')}</p>
      </div>
