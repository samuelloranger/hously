...
import { type ExternalNotificationService, type NotificationTemplate } from '@hously/shared';
...
export function ServiceList({ services, templatesByService, isLoading, onEditTemplate }: ServiceListProps) {
  ...
  const handleRegenerateToken = async (service: ExternalNotificationService) => {
  ...
  const handleToggleNotifyAdminsOnly = async (service: ExternalNotificationService) => {
    setLoadingServiceId(service.id);
    try {
      ...
      toast.success(t('settings.externalNotifications.notifyAdminsOnlyUpdated'));
      queryClient.invalidateQueries({
        queryKey: queryKeys.externalNotifications.services(),
      });
    } catch (error: any) {
  ...
  if (isLoading) {
    return <div className="text-center py-8 text-neutral-500 dark:text-neutral-400">{t('common.loading')}</div>;
  }
  ...
  const getTemplatesForService = (serviceId: number, language: string): NotificationTemplate[] => {
    const templates = templatesByService[serviceId] || [];
    return templates.filter(t => t.language === language);
  };

  const groupTemplatesByEventType = (templates: NotificationTemplate[]): Record<string, NotificationTemplate[]> => {
  ...
  const getAllTemplatesByEventType = (serviceId: number): Record<string, NotificationTemplate[]> => {
    const templates = templatesByService[serviceId] || [];
        ...
        return (
          <ServiceCard
            ...
            onEditTemplate={eventType => {
              ...
              const template = templates.find(t => t.language === currentLanguage) || templates[0];
              if (template) {
