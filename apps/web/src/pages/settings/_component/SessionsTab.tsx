...
export function SessionsTab() {
  ...
  const { data: pushTokensData, isLoading: loadingPushTokens } = useAdminPushTokens();
  ...
  const handleRevokeUserSessions = async (userId: number, userEmail: string) => {
    if (!confirm(t('settings.sessions.revokeAllConfirm', { email: userEmail }))) return;
    try {
  ...
  const sessionsByUser = (sessionsData?.sessions ?? []).reduce<Record<number, { email: string; count: number }>>(
    (acc, s) => {
  ...
  return (
    <div className="animate-in fade-in slide-in-from-right-4 duration-300 space-y-6" key="sessions-tab">
      {/* Active Sessions */}
      <SectionCard
                    ...
                    <td className="py-3 px-4 text-right">
                      <div className="flex items-center justify-end gap-2">
                        {sessionsByUser[session.user_id]?.count > 1 && (
                          <button
                            onClick={() => handleRevokeUserSessions(session.user_id, session.user_email)}
                ...
                {webPushData.subscriptions.map(sub => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="py-3 px-4 text-neutral-600 dark:text-neutral-400">
                        {sub.created_at ? formatDateTime(sub.created_at, i18n.language) : '—'}
                      </td>
                      <td className="py-3 px-4 text-right">
                        <button
