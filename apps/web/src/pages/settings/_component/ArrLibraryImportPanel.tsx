          ...
          <div className="space-y-2">
            ...
            {status.result.radarr && (
              <div className="rounded-lg bg-neutral-50 dark:bg-neutral-800/60 px-3 py-2 text-[10px] text-neutral-600 dark:text-neutral-400">
                ...
                {status.result.radarr.imported} imported ·{" "}
                {status.result.radarr.already_existed} existed ·{" "}
                {status.result.radarr.files_scanned} MediaInfo scans
                {status.result.radarr.errors.length > 0 && (
                  <span className="text-red-500">
                    {" "}
                    · {status.result.radarr.errors.length} errors
                  </span>
                )}
              </div>
            )}
            {status.result.sonarr && (
              <div className="rounded-lg bg-neutral-50 dark:bg-neutral-800/60 px-3 py-2 text-[10px] text-neutral-600 dark:text-neutral-400">
                <span className="font-medium text-neutral-700 dark:text-neutral-300">
                  Sonarr:{" "}
                </span>
                {status.result.sonarr.imported_shows} shows ·{" "}
                {status.result.sonarr.imported_episodes} episodes ·{" "}
                {status.result.sonarr.files_scanned} MediaInfo scans
                {status.result.sonarr.errors.length > 0 && (
                  <span className="text-red-500">
                    {" "}
                    · {status.result.sonarr.errors.length} errors
                  </span>
                )}
              </div>
            )}
