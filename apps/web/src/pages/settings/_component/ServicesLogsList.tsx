...
import { useExternalNotificationLogs } from '@/hooks/useExternalNotifications';
...
export function ServicesLogsList() {
  ...
  const { data: logsData, isLoading, refetch, isFetching } = useExternalNotificationLogs();
                    ...
                    <td className="px-6 py-4 text-sm">
                      <CollapsibleTrigger asChild>
                        <button className="flex items-center gap-2 text-blue-600 dark:text-blue-400 hover:underline font-medium whitespace-nowrap min-w-[160px]">
                          {isOpen
                            ? t('settings.externalNotifications.logs.hidePayload')
                            : t('settings.externalNotifications.logs.viewPayload')}
                          <ChevronDown
