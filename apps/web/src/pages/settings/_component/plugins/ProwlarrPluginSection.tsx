...
import { useProwlarrPlugin, useUpdateProwlarrPlugin } from '@/hooks/usePlugins';
...
export function ProwlarrPluginSection() {
  ...
  return <ProwlarrPluginSectionImpl key={data?.plugin?.type ?? 'pending'} data={data} isLoading={isLoading} />;
}

function ProwlarrPluginSectionImpl({
  data,
  isLoading,
}: {
  data: ReturnType<typeof useProwlarrPlugin>['data'];
  isLoading: boolean;
}) {
