...
import { useUpdateProfile, useChangePassword, useUploadAvatar } from '@/hooks/useUsers';
...
export function ProfileForm() {
  ...
  const onProfileSubmit = async (data: ProfileFormData) => {
    ...
    try {
      ...
      toast.error((error instanceof Error ? error.message : null) || t('settings.profile.updateError'));
    }
  };
  ...
  const onAvatarChange = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      toast.error((error instanceof Error ? error.message : null) || t('settings.profile.avatarUpdateError'));
    } finally {
      event.target.value = '';
    }
  };
  ...
  const onCropApply = async () => {
    ...
    const downscaleFactor = Math.min(1, MAX_CROPPED_DIMENSION_PX / Math.max(sourceWidth, sourceHeight));
    const outputWidth = Math.max(1, Math.round(sourceWidth * downscaleFactor));
    const outputHeight = Math.max(1, Math.round(sourceHeight * downscaleFactor));
    ...
    <div className="space-y-8">
      ...
      <form onSubmit={handleProfileSubmit(onProfileSubmit)} className="space-y-6">
        ...
        <div>
          ...
          <div className="flex items-center gap-4">
            ...
            <label className="inline-flex cursor-pointer items-center px-4 py-2 bg-white dark:bg-neutral-700 border border-neutral-300 dark:border-neutral-600 rounded-lg text-neutral-900 dark:text-neutral-100 hover:bg-neutral-50 dark:hover:bg-neutral-600 transition-colors">
              <span>
                {uploadAvatarMutation.isPending ? t('settings.profile.uploading') : t('settings.profile.uploadPhoto')}
              </span>
              <input
                type="file"
        ...
        <div>
          ...
          />
          <p className="mt-1 text-sm text-neutral-500 dark:text-neutral-400">{t('settings.profile.emailReadOnly')}</p>
        </div>
        ...
        <div>
          ...
          <input
            ...
            {...registerPassword('new_password', {
              ...
              minLength: {
                value: 8,
                message: t('settings.profile.passwordMinLength') || 'Password must be at least 8 characters',
              },
            })}
          ...
          <p className="mt-1 text-sm text-neutral-500 dark:text-neutral-400">{t('settings.profile.passwordHelp')}</p>
        </div>
        ...
        <div>
          ...
          <input
            ...
            {...registerPassword('confirm_password', {
              ...
              validate: value =>
                value === newPassword || t('settings.profile.passwordMismatch') || 'Passwords do not match',
            })}
      ...
      <Dialog isOpen={Boolean(selectedImageUrl)} onClose={onCropCancel} title={t('settings.profile.cropPhoto')}>
        <div className="space-y-4">
          ...
          <div className="flex gap-2 justify-end">
            ...
            >
              {uploadAvatarMutation.isPending ? t('settings.profile.uploading') : t('settings.profile.applyCrop')}
            </button>
          </div>
        </div>
      </Dialog>
