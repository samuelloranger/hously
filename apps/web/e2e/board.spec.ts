...
async function createTask(page: Page, title: string, status = "todo") {
  ...
  const statusSelect = page
    .locator("select")
    .filter({ hasText: /To Do|Todo|backlog/i })
    .first();
...
test.describe("Task drawer", () => {
  ...
  test("drawer shows Status, Priority, Assignee, Start date, Due date, Tags fields", async ({
    page,
  }) => {
    await page.getByText(taskTitle).click();
    const drawer = page.locator('[role="dialog"]');
  ...
  test("can change task priority in the drawer", async ({ page }) => {
    ...
    const prioritySelect = drawer
      .locator("select")
      .filter({ hasText: /Medium|High|Low|Urgent/i })
      .first();
  ...
  test("can set a due date in the drawer", async ({ page }) => {
    ...
    await page.locator('[role="dialog"] button[aria-label="Close"]').click();
    await page.getByText(taskTitle).click();
    await expect(drawer.locator('input[type="date"]').nth(1)).toHaveValue(
      "2099-12-31",
    );
  });
...
test.describe("Backlog view", () => {
  ...
  test("switches to backlog view when Backlog tab is clicked", async ({
    page,
  }) => {
    await page.getByRole("button", { name: /Backlog/ }).click();
...
test.describe("Filters", () => {
  ...
  test("priority filter hides non-matching tasks", async ({ page }) => {
    ...
    await page.getByRole("button", { name: "Filters" }).click();
    const filterPriority = page
      .locator("select")
      .filter({ hasText: /Any priority/i });
  ...
  test("Clear button resets filters", async ({ page }) => {
    ...
    await expect(
      page.locator("select").filter({ hasText: /Any priority/i }),
    ).toBeVisible();
  });
});
